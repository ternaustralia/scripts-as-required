...
<xsl:stylesheet version="2.0" 
    ...
    <!-- stylesheet to convert data.vic.gov.au xml (transformed from json with python script) to RIF-CS -->
    <xsl:output method="xml" version="1.0" encoding="UTF-8" omit-xml-declaration="yes" indent="yes"/>
    <xsl:strip-space elements='*'/>
    <xsl:param name="global_originatingSource" select="'http://data.vic.gov.au'"/>
    <xsl:param name="global_baseURI" select="'http://data.vic.gov.au/'"/>
    ...
    <xsl:template match="result" mode="collection">
        ...
        <xsl:variable name="metadataURL">
            ...
            <xsl:if test="string-length($name)">
                <xsl:value-of select="concat($global_baseURI, 'data/dataset?q=', $name)"/>
            </xsl:if>
        </xsl:variable>
    ...
    <xsl:template match="name" mode="collection_identifier">
        <xsl:if test="string-length(normalize-space(.))">
              <identifier>
                ...
                <xsl:value-of select="concat($global_baseURI, 'data/dataset?q=', normalize-space(.))"/>
            </identifier>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="organization" mode="collection_related_object">
        <xsl:if test="string-length(normalize-space(title))">
            <relatedObject>
                <key>
                    <xsl:value-of select="concat($global_group,'/', translate(lower-case(normalize-space(title)),' ',''))"/>
                </key>
                <relation>
                    <xsl:attribute name="type">
    ...
    <xsl:template match="author" mode="collection_related_object">
        <xsl:if test="string-length(normalize-space(.))">
            <relatedObject>
                <key>
                    <xsl:value-of select="concat($global_group,'/', translate(normalize-space(lower-case(normalize-space(.))),' ',''))"/>
                </key>
                <relation>
                    <xsl:attribute name="type">
                    ...
                    <xsl:when test="string-length($serviceUrl) > 0">
                        ...
                        <relatedInfo type="service">
                            ...
                            <relation type="isSupportedBy">
                                <description><xsl:value-of select="concat(name, ' via service')"/></description>
    ...
    <xsl:template match="result" mode="party_author">
        ...
        <xsl:if test="string-length($name) > 0">
            <registryObject group="{$global_group}">
                ...
                <key>
                    <xsl:value-of select="concat($global_group, '/', translate(lower-case($name),' ',''))"/>
                </key>
                    ...
                    <service type="webservice">
                            ...
                            <namePart>
                                <xsl:choose>
                                    <xsl:when test="string-length($serviceName)">
                                        <xsl:value-of select="concat($serviceName, ' - ', $organizationTitle)"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="concat('Service linked to ', $organizationTitle)"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </namePart>
                        </name>
                        ...
                        <xsl:if test="string-length($organizationName)">
                            <relatedInfo type="party">
                                <identifier type="uri">
                                    <xsl:value-of select="concat($global_baseURI,'data/organization/', $organizationName)"/>
                                </identifier>
                            </relatedInfo>
                        </xsl:if>
                    </service>
