...
<xsl:stylesheet version="2.0" 
    ...
    <!-- stylesheet to convert default.ckan.feed xml (transformed from json with python script) to RIF-CS -->
    <xsl:output method="xml" version="1.0" encoding="UTF-8" omit-xml-declaration="yes" indent="yes"/>
    <xsl:strip-space elements='*'/>
    <xsl:param name="global_originatingSource" select="'http://default.ckan.feed'"/>
    <xsl:param name="global_baseURI" select="'http://default.ckan.feed/'"/>
    <xsl:param name="global_group" select="'default.ckan.feed'"/>
    <xsl:param name="global_contributor" select="'default.ckan.feed'"/>
    <xsl:param name="global_publisherName" select="'default.ckan.feed'"/>
