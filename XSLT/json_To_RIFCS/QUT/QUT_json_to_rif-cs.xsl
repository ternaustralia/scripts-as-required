...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="global_baseURI" select="'http://130.56.252.146/'"/>
    <xsl:param name="global_groupAcronym" select="'QUT'"/>
    <xsl:param name="global_group" select="'Queensland University of Technology (QUT)'"/>
    <xsl:param name="global_contributor" select="'Queensland University of Technology (QUT)'"/>
    <xsl:param name="global_publisherName" select="'Queensland University of Technology (QUT)'"/>
    ...
    <xsl:template match="datasets/result" mode="collection">
        ...
        <registryObject>
            ...
            <collection>
                ...
                <xsl:apply-templates select="resources" mode="collection_relatedInfo"/>

                <xsl:apply-templates select="." mode="relatedInfo_services"/>
    ...
    <xsl:template match="id" mode="collection_key">
        <xsl:if test="string-length(normalize-space(.))">
            <key>
                <xsl:value-of select="concat($global_groupAcronym,'/', lower-case(normalize-space(.)))"/>
            </key>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="organization" mode="collection_related_object">
        <xsl:if test="string-length(normalize-space(title))">
            <relatedObject>
                <key>
                    <xsl:value-of
                        select="concat($global_groupAcronym,'/', translate(lower-case(normalize-space(title)),' ',''))"
                    />
                </key>
                <relation>
                    <xsl:attribute name="type">
    ...
    <xsl:template match="value" mode="collection_coverage_spatial">
        ...
        <xsl:variable name="coordinate_sequence" as="xs:string*">
            <xsl:if test="((contains($spatial, 'Polygon')) or contains($spatial, 'Point')) and contains($spatial, 'coordinates')">
            ...
            <xsl:if test="string-length($url)">
                <xsl:variable name="serviceUrl">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:if
                                test="contains(lower-case(normalize-space(resource_type)), 'api')">
                                <xsl:choose>
                                    <xsl:when test="contains($url, '?')">
                                        <!-- Indicates parameters -->
                    ...
                    <xsl:when test="string-length($serviceUrl) > 0">
                        ...
                        <relatedInfo type="service">
                            ...
                            <relation type="isSupportedBy">
                                <description>
                                    <xsl:value-of select="concat(name, ' via service')"/>
                                </description>
                                <url>
                                    <xsl:value-of select="$url"/>
                                </url>
    ...
    <xsl:template match="organization">
        ...
        <xsl:if test="string-length($title) > 0">
            <registryObject group="{$global_group}">
                ...
                <key>
                    <xsl:value-of
                        select="concat($global_groupAcronym, '/', translate(lower-case($title),' ',''))"/>
                </key>
                ...
                <party type="group">
                    ...
                    <xsl:if test="string-length(normalize-space(image_url))">
                        <description>
                            ...
                            <xsl:value-of
                                select="normalize-space(image_url)"/>
                        </description>
                    </xsl:if>
