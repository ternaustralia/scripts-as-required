...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:param name="global_originatingSource" select="'National Environmental Information Infrastructure'"/> <!-- Only used as originating source if organisation name cannot be determined from Point Of Contact -->
    <xsl:param name="global_group" select="'National Environmental Information Infrastructure'"/> 
    <xsl:param name="global_baseURI" select="'http://neii.bom.gov.au'"/>
    <xsl:param name="global_catalogueURI" select="'http://neii.gov.au/catalogue'"/>
    <xsl:param name="global_rightsURI" select="'http://neii.gov.au/copyright'"/>
    ...
    <xsl:template match="*:MD_Metadata">
        ...
        <xsl:variable name="locationURL_sequence" as="xs:string*">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="$global_catalogueURI"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <registryObject>
                ...
                <!-- Request  by NEII to map all records to datasets at the moment -->
                ...
                <xsl:variable name="registryObjectTypeSubType_sequence" as="xs:string*">
                    <xsl:choose>
                                ...
                                <xsl:when test="contains(lower-case($scopeCode), 'collectionhardware')">
                ...
                <xsl:if test="(count($registryObjectTypeSubType_sequence) = 2)">
                    <xsl:element name="{$registryObjectTypeSubType_sequence[1]}">
                       ...
                       <xsl:apply-templates
                        ...
                        <!-- the best we can do for rights at the moment -->
                        <rights>
                            <rightsStatement rightsUri="{$global_rightsURI}"/>
                        </rights>
                        ...
                        <xsl:apply-templates select="descendant::*:resourceConstraints/*:MD_LegalConstraints" mode="registryObject_rights_licence_type_and_uri"/>
                        
                        <xsl:apply-templates select="*[contains(lower-case(name()),'identification')]" mode="registryObject_rights_access"/>
                ...
                <location>
                    <address>
                        <electronic>
                            ...
                            <value>
                                    <!-- use the catalogue URL until a URL per collection is available (requires technical
                                        development of the NEII catalogue -->
                                <xsl:value-of select="."/>
                            </value>
                        </electronic>
                    </address>
   ...
   <xsl:template match="*:MD_TopicCategoryCode" mode="registryObject_subject">
        ...
        <xsl:for-each-group select="*:transferOptions/*:MD_DigitalTransferOptions/*:onLine/*:CI_OnlineResource" group-by="*:linkage/*:URL">
            ...
            <xsl:if test="(string-length($protocol) > 0) and not(contains(lower-case($protocol), 'metadata-url'))">
                ...
                <xsl:if test="string-length($identifierValue) > 0">
                    <relatedInfo>
                        <xsl:choose>
                            <xsl:when test="contains(lower-case($protocol), 'ogc')">
                                <xsl:attribute name="type">
                                ...
                                <relation>
                                    ...
                                    <xsl:if test="contains($identifierValue, '?') and (string-length(substring-after($identifierValue, '?')) > 0)">
                                        <url>
                                            <xsl:value-of select="$identifierValue"/>    
                                        </url>
                                    </xsl:if>
    ...
    <!--xsl:template match="*:childIdentifier" mode="registryObject_relatedInfo">
        ...
        <xsl:if test="string-length($identifier) > 0">
            <relatedInfo type="collection">
                <identifier type="uri">
                    <xsl:value-of
                        select="concat($global_baseURI, '/nei-cat-searchui/show/', $identifier)"
                    />
                </identifier>
                <relation>
                    <xsl:attribute name="type">
    ...
    <xsl:template name="populateLicence">
       ...
       <xsl:message select="concat('count $licenseCodelist : ', count($licenseCodelist))"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="(string-length($licenceText) > 0) and ($licenceText != 'None')">
                    <rights>
                        <licence>
                            <xsl:value-of select="$licenceText"/>
                        </licence>
    ...
    <xsl:template match="*:MD_LegalConstraints" mode="registryObject_rights_licence_type_and_uri">
