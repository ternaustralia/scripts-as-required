...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:param name="global_defaultOriginatingSource" select="'NSW Office of Environment and Heritage (OEH)'"/>
    <xsl:param name="global_acronym" select="'OEH'"/>
    <xsl:param name="global_originatingSource" select="'NSW Office of Environment and Heritage (OEH)'"/> <!-- Only used as originating source if organisation name cannot be determined from Point Of Contact -->
    <xsl:param name="global_group" select="'NSW Office of Environment and Heritage (OEH)'"/> 
    <xsl:param name="global_baseURI" select="'mapdata.environment.nsw.gov.au'"/>
    <xsl:param name="global_KeyOEH" select="'OEH/NSWOfficeofEnvironmentandHeritage(OEH)'"/>
    <xsl:param name="global_NameOEH" select="'NSW Office of Environment and Heritage (OEH)'"/>
   ...
   <!-- RegistryObject - Identifier Element  -->
    ...
    <xsl:template match="*:CI_ResponsibleParty" mode="registryObject_related_object">
         <relatedObject>
             <key> 
                 <xsl:variable name="mappedKey" select="custom:getMappedKey(normalize-space(current-grouping-key()))"/>
                 <xsl:choose>
            ...
            <xsl:if test="string-length($useLimitation) > 0">
                <rights>
                   ...
                   <xsl:for-each
                        select="$licenseCodelist/gmx:CT_CodelistCatalogue/gmx:codelistItem/gmx:CodeListDictionary[@gml:id='LicenseCode']/gmx:codeEntry/gmx:CodeDefinition">
                        <xsl:if test="string-length(normalize-space(gml:remarks)) > 0">
                            <xsl:if test="contains(lower-case($useLimitation), lower-case(gml:remarks))">
                                <licence>
                                    <xsl:attribute name="type" select="gml:identifier"/>
                        ...
                        <xsl:when test="count($allContributorName_sequence) > 0">
                            <xsl:for-each select="distinct-values($allContributorName_sequence)">
                                <xsl:if test="
                                    ...
                                    <contributor>
                                        <namePart>
                                            <xsl:variable name="mappedName" select="custom:getMappedName(.)"/>
                                            <xsl:choose>
                                                <xsl:when test="string-length($mappedName) > 0">
            ...
            <xsl:when test="boolean(custom:createObject(translate(normalize-space(current-grouping-key()),' ','')))">
                ...
                <key>
                 ...
                 <party type="{$type}">
                     <name type="primary">
                         <namePart>
                             <xsl:variable name="mappedName" select="custom:getMappedName(normalize-space(current-grouping-key()))"/>
                             <xsl:choose>
                                 <xsl:when test="string-length($mappedName) > 0">
                                     <xsl:value-of select="$mappedName"/>
                                 </xsl:when>
                     ...
                     <xsl:choose>
                         <xsl:when test="contains(lower-case($type), 'person')">
                             <xsl:choose>
                                 <xsl:when test="string-length(normalize-space(*:organisationName)) > 0">
                                     ...
                                     <relatedObject>
                                         <key>
                                             <xsl:variable name="mappedKey" select="custom:getMappedKey(normalize-space(*:organisationName))"/>
                                             <xsl:choose>
                                                <xsl:when test="string-length($mappedKey) > 0">
                                                    <xsl:value-of select="$mappedKey"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                   <xsl:value-of select="concat($global_acronym,'/', translate(normalize-space(*:organisationName),' ',''))"/>
                                                </xsl:otherwise>
                                             </xsl:choose>
                                         </key>
   ...
   <xsl:function name="custom:doiFromIdentifiers">
    ...
    <xsl:function name="custom:getMappedKey" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="
                contains(lower-case($input), 'office of environment and heritage')">
                <xsl:value-of select="$global_KeyOEH"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="''"/>
            </xsl:otherwise> 
    ...
    <xsl:function name="custom:getMappedName" as="xs:string">
        <xsl:param name="input" as="xs:string"/>
        <xsl:choose>
            <xsl:when test="
                contains(lower-case($input), 'office of environment and heritage')">
                <xsl:value-of select="$global_NameOEH"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="''"/>
            </xsl:otherwise> 
