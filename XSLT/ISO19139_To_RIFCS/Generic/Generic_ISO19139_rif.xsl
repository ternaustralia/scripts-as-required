...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- Collection - Coverage Spatial Element -->
    ...
    <xsl:template match="gmd:MD_LegalConstraints" mode="collection_rights_rightsStatement">
        <xsl:message>MD_LegalConstraints</xsl:message>
    ...
    <xsl:template match="gmd:MD_SecurityConstraints" mode="collection_rights_rightsStatement">
        <xsl:message>MD_SecurityConstraints</xsl:message>
        <xsl:for-each select="gmd:useLimitation">
            <!-- If there is text in other contraints, use this; otherwise, do nothing -->
            <xsl:if test="string-length(normalize-space(.))">
                <rights>
    ...
    <xsl:template name="collection_citationMetadata_citationInfo">
        ...
        <xsl:variable name="identifier" select="normalize-space(gmd:identifier/gmd:MD_Identifier/gmd:code)"/>
        <xsl:variable name="lowerIdentifier">
            <xsl:call-template name="toLower">
                <xsl:with-param name="inputString" select="$identifier"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="identifierToUse">
            <xsl:choose>
                <xsl:when test="string-length($identifier) and contains($lowerIdentifier, 'doi')">
        ...
        <xsl:variable name="typeToUse">
            <xsl:choose>
                <xsl:when test="string-length($identifierToUse) and contains($lowerIdentifier, 'doi')">
                    <xsl:text>doi</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:text>uri</xsl:text>
    ...
    <xsl:template name="party">
        ...
        <registryObject group="{$global_group}">
            ...
            <xsl:variable name="typeToUse">
                ...
                <!-- Comment line above and change and uncomment lines below if you need to add organisation substrings to
                    ensure that type is 'group', not 'person' - this was necessary in some cases where an organisation
                    name was coming through in an individual's name -->
                <!--xsl:choose>
    ...
    <xsl:template name="transform">
        ...
        <!-- Comment line above, and change and uncomment lines below in case you need acronyms transformed to full titles -->
        <!--xsl:choose>
