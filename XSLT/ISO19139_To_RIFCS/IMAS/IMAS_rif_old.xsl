...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:param name="global_group" select="'IMAS'"/> 
    ...
    <xsl:template match="*:MD_Metadata">
        ...
        <xsl:variable name="originatingSource">
         ...
         <registryObject>
                ...
                <xsl:variable name="registryObjectTypeSubType_sequence" as="xs:string*" select="custom:getRegistryObjectTypeSubType_IMAS($scopeCode)"/>
                <xsl:if test="(count($registryObjectTypeSubType_sequence) = 2)">
                    <xsl:element name="{$registryObjectTypeSubType_sequence[1]}">
                        ...
                        <xsl:for-each-group
                            select="*:identificationInfo/*/*:citation/*:CI_Citation/*:citedResponsibleParty/*:CI_ResponsibleParty[string-length(normalize-space(gmd:individualName)) > 0] |
                            *:distributionInfo/*:MD_Distribution/*:distributor/*:MD_Distributor/*:distributorContact/*:CI_ResponsibleParty[string-length(normalize-space(gmd:individualName)) > 0] |
                            *:identificationInfo/*/*:pointOfContact/*:CI_ResponsibleParty[string-length(normalize-space(gmd:individualName)) > 0]"
                        ...
                        <xsl:apply-templates select="*:identificationInfo/*/*:descriptiveKeywords/*:MD_Keywords/*:keyword"
                            mode="registryObject_related_subject"/>
   ...
   <!-- RegistryObject - Identifier Element  -->
    ...
    <!-- RegistryObject - Related Object Element  -->
    <xsl:template match="*:keyword" mode="registryObject_related_subject">
    ...
    <xsl:template match="*:CI_ResponsibleParty" mode="registryObject_related_object">
        <xsl:message select="concat('Related object current-grouping-key(): ', current-grouping-key())"/>
        <xsl:message select="concat('IndividualName name: ', gmd:individualName)"/> 
   ...
   <xsl:template match="*:MD_TopicCategoryCode" mode="registryObject_subject">
                ...
                <citationMetadata>
                    ...
                    <xsl:variable name="publisherToUse">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="$global_defaultPublisher"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
    ...
    <xsl:function name="custom:getRegistryObjectTypeSubType_IMAS" as="xs:string*">
