...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:param name="global_acronym" select="'QSpatial'"/>
    <xsl:param name="global_originatingSource" select="'Queensland Spatial Catalogue - QSpatial'"/> <!-- Only used as originating source if organisation name cannot be determined from Point Of Contact -->
    <xsl:param name="global_group" select="'Queensland Spatial Catalogue'"/> 
    <xsl:param name="global_baseURI" select="'qldspatial.information.qld.gov.au'"/>
    <xsl:param name="global_path" select="'/catalogue/custom/detail.page?fid='"/>
   ...
   <xsl:template match="*[contains(lower-case(name()),'identification')]" mode="registryObject">
       ...
       <xsl:apply-templates select="*:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox" mode="registryObject_coverage_spatial"/>
       <xsl:apply-templates select="*:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_BoundingPolygon/gmd:polygon" mode="registryObject_coverage_spatial"/>
       
        <xsl:apply-templates
            select="*:extent/gmd:EX_Extent/gmd:temporalElement/gmd:EX_TemporalExtent"
