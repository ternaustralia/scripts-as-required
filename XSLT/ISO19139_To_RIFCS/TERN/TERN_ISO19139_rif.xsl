...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="gmd:MD_Metadata">
        ...
        <xsl:variable name="locationURL">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="concat('http://', $global_baseURI, $global_metadataLandingPageSubPath, $fileIdentifier)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <registryObject>
                ...
                <xsl:if test="(count($registryObjectTypeSubType_sequence) = 2)">
                    <xsl:element name="{$registryObjectTypeSubType_sequence[1]}">
                        ...
                        <xsl:for-each-group
                            select="descendant::gmd:citation/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[(ancestor::gmd:MD_DataIdentification | ancestor::gmd:MD_ServiceIdentification) and ((string-length(normalize-space(gmd:organisationName))) > 0) and ((string-length(normalize-space(gmd:individualName))) = 0)] |
                            ...
                            descendant::gmd:pointOfContact/gmd:CI_ResponsibleParty[(ancestor::gmd:MD_DataIdentification | ancestor::gmd:MD_ServiceIdentification) and ((string-length(normalize-space(gmd:organisationName))) > 0) and ((string-length(normalize-space(gmd:individualName))) = 0)]"
                       ...
                       <xsl:apply-templates
                        ...
                        <xsl:apply-templates
                            select="descendant::gmd:resourceConstraints/*:MD_CreativeCommons[(ancestor::gmd:MD_DataIdentification | ancestor::gmd:MD_ServiceIdentification) and exists(*:licenseLink)]"
                            mode="registryObject_rights_licence_creative"/>
    
                        <xsl:apply-templates
                            select="descendant::gmd:resourceConstraints/*:MD_CreativeCommons[ancestor::gmd:MD_DataIdentification | ancestor::gmd:MD_ServiceIdentification]"
   ...
   <xsl:template match="gmd:MD_TopicCategoryCode" mode="registryObject_subject">
    ...
    <xsl:template match="gmd:childIdentifier" mode="registryObject_relatedInfo">
        ...
        <xsl:if test="string-length($identifier) > 0">
            <relatedInfo type="collection">
                <identifier type="uri">
                    <xsl:value-of
                        select="concat('http://', $global_baseURI, $global_metadataLandingPageSubPath, $identifier)"
                    />
                </identifier>
                <relation>
                    <xsl:attribute name="type">
    ...
    <xsl:function name="custom:rights">
        ...
        <xsl:for-each select="$currentNode/gmd:otherConstraints">
            ...
            <xsl:if test="string-length($otherConstraints) > 0">
                ...
                <xsl:choose>
                    <xsl:when test="
                        contains(lower-case($otherConstraints), 'licenc') or 
                        contains(lower-case($otherConstraints), 'licens')">
                        <xsl:variable name="licenceType_sequence" select="custom:licenceType($otherConstraints)"/>
                        <xsl:variable name="lastIndex" select="count($licenceType_sequence)" as="xs:integer"/>
                        <xsl:message select="concat('lastIndex : ', $lastIndex)"/>
                        <xsl:message select="concat('count($licenceType_sequence) : ', count($licenceType_sequence))"/>
                        <xsl:message select="concat('licenceType_sequence[last] : ', $licenceType_sequence[$lastIndex])"/>
                        <rights>
                            <licence>
                                <xsl:if test="string-length($licenceType_sequence[$lastIndex]) > 0">
                                 <xsl:attribute name="type">
                                     <xsl:value-of select="$licenceType_sequence[$lastIndex]"/>
                                 </xsl:attribute>
                             </xsl:if>
    ...
    <xsl:function name="custom:licenceType" as="xs:string*">
        <xsl:param name="text"/>
        
        <xsl:for-each
            select="$licenseCodelist/gmx:CT_CodelistCatalogue/gmx:codelistItem/gmx:CodeListDictionary[@gml:id='LicenseCode']/gmx:codeEntry/gmx:CodeDefinition">
            <xsl:message select="concat('lower-case($text) : ', lower-case($text))"/>
            <xsl:message select="concat('lower-case(gml:identifier) : ', lower-case(gml:identifier))"/>
            <xsl:if test="contains(lower-case($text), lower-case(gml:identifier))">
                <xsl:message select="concat('contains gml:identifer: ', gml:identifier)"/>
                <xsl:copy-of select="gml:identifier"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:function>
    ...
    <xsl:template name="registryObject_citationMetadata_citationInfo">
        ...
        <xsl:variable name="citedResponsibleParty_sequence" select="$CI_Citation/gmd:citedResponsibleParty" as="node()*"></xsl:variable>
