...
<xsl:stylesheet version="2.0" 
    ...
    xmlns:auscopeFunc="http://nci.nowhere.yet"
    ...
    <xsl:template match="/">
        <registryObjects>
            ...
            <xsl:apply-templates select="//gmd:MD_Metadata" mode="AuScope"/>
        </registryObjects>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata" mode="AuScope">
        ...
        <registryObject>
            ...
            <xsl:element name="{auscopeFunc:registryObjectClass(gmd:hierarchyLevel/*[contains(lower-case(name()),'scopecode')]/@codeListValue)}">
    
                <xsl:attribute name="type" select="auscopeFunc:registryObjectType(gmd:hierarchyLevel/*[contains(lower-case(name()),'scopecode')]/@codeListValue)"/>
                        
                <xsl:if test="auscopeFunc:registryObjectClass(gmd:hierarchyLevel/*[contains(lower-case(name()),'scopecode')]/@codeListValue) = 'collection'">
                        <xsl:if test="
   ...
   <xsl:template match="*[contains(lower-case(name()),'identification')]" mode="registryObject">
       ...
       <xsl:apply-templates select="gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_BoundingPolygon/gmd:polygon" mode="registryObject_coverage_spatial"/>
        ...
        <xsl:if test="auscopeFunc:registryObjectClass(ancestor::gmd:MD_Metadata/gmd:hierarchyLevel/*[contains(lower-case(name()),'scopecode')]/@codeListValue) = 'collection'">
   ...
   <!-- RegistryObject - RelatedInfo Element  -->
    ...
    <xsl:function name="auscopeFunc:registryObjectClass" as="xs:string">
    ...
    <xsl:function name="auscopeFunc:registryObjectType" as="xs:string*">
        <xsl:param name="scopeCode_sequence" as="xs:string*"/>
        <xsl:choose>
            <xsl:when test="count($scopeCode_sequence) > 0">
                <xsl:choose>
