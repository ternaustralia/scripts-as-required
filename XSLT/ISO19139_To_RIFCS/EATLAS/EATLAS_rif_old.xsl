...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="global_originatingSource" select="'http://eatlas.org.au:80'"/>
    <xsl:param name="global_group" select="'eAtlas'"/>
    <xsl:param name="global_publisherName" select="'eAtlas'"/>
    ...
    <xsl:template match="mcp:MD_Metadata">
        ...
        <xsl:if
            test="contains(lower-case($metadataURL), 'http://eatlas.org.au')">
            ...
            <registryObject>
                ...
                <xsl:element name="{$registryObjectType}">
                    ...
                    <xsl:if test="string-length($registryObjectSubType) > 0">
                    ...
                    <xsl:for-each-group
                        select="gmd:identificationInfo/mcp:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:individualName)) > 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)] |
                     gmd:distributionInfo/gmd:MD_Distribution/gmd:distributor/gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:individualName)) > 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue))) > 0] |
                     gmd:identificationInfo/mcp:MD_DataIdentification/gmd:pointOfContact/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:individualName)) > 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)]"
                    ...
                    <xsl:for-each-group
                        select="gmd:identificationInfo/mcp:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[((string-length(normalize-space(gmd:organisationName)) > 0) and (string-length(normalize-space(gmd:individualName))) = 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)] |
                     gmd:distributionInfo/gmd:MD_Distribution/gmd:distributor/gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty[((string-length(normalize-space(gmd:organisationName)) > 0) and (string-length(normalize-space(gmd:individualName))) = 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)] |
                     gmd:identificationInfo/mcp:MD_DataIdentification/gmd:pointOfContact/gmd:CI_ResponsibleParty[((string-length(normalize-space(gmd:organisationName)) > 0) and (string-length(normalize-space(gmd:individualName))) = 0) and 
                     (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)]"
            ...
            <xsl:for-each-group
                select="gmd:identificationInfo/mcp:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:organisationName)) > 0) and 
             (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)] |
             gmd:distributionInfo/gmd:MD_Distribution/gmd:distributor/gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:organisationName)) > 0) and 
             (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)] |
             gmd:identificationInfo/mcp:MD_DataIdentification/gmd:pointOfContact/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:organisationName)) > 0) and 
             (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)]"
                group-by="gmd:organisationName">
                <xsl:call-template name="party">
    ...
    <xsl:template match="gmd:MD_DigitalTransferOptions" mode="registryObject_originating_source">
        <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource">
            <xsl:if test="contains(lower-case(gmd:protocol), 'http--metadata-url')">
                <originatingSource>
    ...
    <xsl:template match="gmd:MD_DigitalTransferOptions" mode="registryObject_identifier">
        <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource">
            <xsl:if test="contains(lower-case(gmd:protocol), 'www:link-1.0-http--metadata-url')">
    ...
    <xsl:template match="gmd:EX_BoundingPolygon" mode="registryObject_coverage_spatial">
        <xsl:for-each select="gmd:polygon/gml:Polygon/gml:exterior/gml:LinearRing/gml:coordinates">
            <xsl:if
            test="boolean(string-length(normalize-space(.)) > 0)">
            <coverage>
                <spatial>
                    <xsl:attribute name="type">
                ...
                <xsl:if test="string-length($identifierValue) > 0">
                    <relatedInfo>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                               <!-- Try to grab a name from the uri - only if not doi -->
                                <xsl:if test="not(contains(lower-case($identifierValue), 'doi'))">
                                    <xsl:message select="concat('identifierValue: ', $identifierValue)" />
                                    <xsl:variable name="match">
                                        <xsl:analyze-string select="normalize-space($identifierValue)"
                                            regex="\(\[\^\\/\]\+\)\(?=\$\)">
                                            <xsl:matching-substring>
                                    ...
                                    <xsl:message select="concat('match: ', $match)" />
    ...
    <xsl:template match="mcp:childIdentifier" mode="registryObject_relatedInfo">
        ...
        <xsl:if test="string-length($identifier) > 0">
            <relatedInfo type="collection">
                <identifier type="uri">
                    <xsl:value-of
                        select="concat('http://eatlas.org.au/geonetwork/srv/en/metadata.show?uuid=', $identifier)"
                    />
                </identifier>
                <relation>
                    <xsl:attribute name="type">
                ...
                <xsl:if test="contains(lower-case($licenseLink), lower-case(gml:remarks))">
                    <rights>
                        <licence>
                            ...
                            <xsl:if test="string-length($licenceText) > 0">
        ...
        <xsl:if test="  (count($principalInvestigatorName_sequence) > 0) or 
            ...
            <citationInfo>
                <citationMetadata>
                    <xsl:choose>
                        <xsl:when
                            test="(string-length(lower-case($identifierType)) > 0) and contains(lower-case($identifierType), 'doi')">
                            <identifier>
                                <xsl:attribute name="type">
                                    <xsl:text>doi</xsl:text>
                                </xsl:attribute>
    ...
    <xsl:template name="party">
        ...
        <registryObject group="{$global_group}">
            ...
            <!-- Use the party type provided, except for exception:
                    Because sometimes eAtlas is used for an author, appearing in individualName,
                    we want to make sure that we use 'group', not 'person', if this anomoly occurs -->

            <xsl:variable name="typeToUse">
                <xsl:choose>
                    <xsl:when test="contains($transformedName, 'eAtlas')">
    ...
    <xsl:template name="getIndividualNameSequence">
        ...
        <xsl:for-each select="tokenize($role_sequence, ',')">
           ...
           <xsl:for-each-group
                 select="$parent/descendant::gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:individualName)) > 0) and 
                 (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)) > 0)]"
                 group-by="gmd:individualName">
