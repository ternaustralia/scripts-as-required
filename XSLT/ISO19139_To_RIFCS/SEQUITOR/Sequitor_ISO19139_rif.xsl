...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="global_originatingSource" select="'http://acef.tern.org.au/geonetwork/srv/en'"/>
    <xsl:param name="global_baseURI" select="'http://acef.tern.org.au'"/>
    <!--xsl:param name="global_activityKeyTERN" select="'TERN-NC-0'"/-->
    <!--xsl:param name="global_activityKeyACEF" select="''"/-->
    <xsl:param name="global_group" select="'Australian Coastal Ecosystems Facility'"/>
    <xsl:param name="global_publisherName" select="'Australian Coastal Ecosystems Facility'"/>
    ...
    <xsl:template match="gmd:MD_Metadata" mode="collection">
        ...
        <registryObject>
            ...
            <collection>
                ...
                <!--relatedObject>
                    <key><xsl:value-of select="$global_activityKeyACEF"/></key>
                    <relation type="isOutputOf"/>
                </relatedObject-->
                
                <!--relatedObject>
                    <key><xsl:value-of select="$global_activityKeyTERN"/></key>
                    <relation type="isOutputOf"/>
                </relatedObject-->
                ...
                <xsl:apply-templates select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:resourceConstraints/gmd:MD_LegalConstraints[
                    exists(gmd:useLimitation) and exists(gmd:useLimitation)]"
                ...
                <xsl:apply-templates select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:resourceConstraints/gmd:MD_LegalConstraints[
                    exists(gmd:accessConstraints) and exists(gmd:accessConstraints)]"
                    mode="collection_rights_accessRights"/> 
                
                <xsl:apply-templates select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:resourceConstraints/gmd:MD_LegalConstraints[
                    exists(gmd:useConstraints) and exists(gmd:useConstraints)]"
    ...
    <xsl:template match="gmd:MD_ScopeCode/@codeListValue" mode="collection_type_attribute">
    ...
    <xsl:template match="gmd:title" mode="collection_name">
        <name>
            <xsl:attribute name="type">
    ...
    <xsl:template match="gmd:CI_Date" mode="collection_dates">
        <xsl:variable name="lowerCode">
            <xsl:call-template name="toLower">
        ...
        <xsl:if test="contains($lowerCode, 'publication')">
            <xsl:if test="string-length(normalize-space(gmd:date/gco:DateTime)) > 0">
                <dates>
                    <xsl:attribute name="type">
   ...
   <!-- Collection - Coverage Spatial Element -->
    ...
    <xsl:template name="collection_citationMetadata_citationInfo">
        ...
        <citationInfo>
            <citationMetadata>
                ...
                <xsl:for-each-group
                    select="gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[(string-length(normalize-space(gmd:individualName))) and 
                    (string-length(normalize-space(gmd:role/gmd:CI_RoleCode/@codeListValue)))]"
                    group-by="gmd:individualName">
                    ...
                    <xsl:variable name="userIsRole" as="xs:boolean*">
                        <xsl:call-template name="isRole">
                            <xsl:with-param name="roles" select="'author,principalInvestigator,originator'"/>
                        </xsl:call-template>
                    </xsl:variable>
                ...
                <xsl:for-each-group
                    select="gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[
    ...
    <xsl:template name="party">
        ...
        <registryObject group="{$global_group}">
            ...
            <!-- Use the party type provided, except for exception:
                Because sometimes "SEQUITOR" or "CSIRO", or "CMAR", is used for an author, appearing in individualName,
                we want to make sure that we use 'group', not 'person', if this anomoly occurs -->
            
            <xsl:variable name="typeToUse">
                <xsl:choose>
                    <xsl:when test="contains($transformedName, 'SEQUITOR') or
                                    contains($transformedName, 'CSIRO') or
                                    contains($transformedName, 'Department') or
                                    contains($transformedName, 'CMAR')">
    ...
    <xsl:template name="transform">
        <xsl:param name="inputString"/>
        <xsl:choose>
            <xsl:when test="normalize-space($inputString) = 'CSIRO'">
                <xsl:text>Commonwealth Scientific and Industrial Research Organisation (CSIRO)</xsl:text>
            </xsl:when>
            <xsl:when test="normalize-space($inputString) = 'CMAR'">
                <xsl:text>CSIRO Marine and Atmospheric Research</xsl:text>
            </xsl:when>
            <xsl:when test="normalize-space($inputString) = 'CSIRO CMAR'">
                <xsl:text>CSIRO Marine and Atmospheric Research</xsl:text>
            </xsl:when>
            <xsl:otherwise>
