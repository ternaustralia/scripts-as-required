                ...
                <xsl:if test="not(contains(lower-case($protocol), 'download')) and not(contains(lower-case($protocol), 'metadata-url'))">
                    ...
                    <xsl:if test="string-length($url) > 0">
                        <relatedInfo>
                            <xsl:attribute name="type">
                                <xsl:choose>
                                    <xsl:when test="contains(lower-case($name), 'publication')">
                                        <xsl:text>publication</xsl:text>
                                    </xsl:when>
                                    <xsl:when test="contains(lower-case($name), 'dataset')">
                            ...
                            <relation>
                                <xsl:attribute name="type">
                                    <xsl:choose>
                                        <xsl:when test="
                                            contains(lower-case($name), 'associated publication')">
                                            <xsl:text>isReferencedBy</xsl:text>
                                        </xsl:when>
                                        <xsl:when test="
                                            contains(lower-case($registryObjectType), 'publication') and
                                            contains(lower-case($name), 'associated dataset')">
